    ...
    if request.method == "POST":
        ...
        if img is not None:
            ...
            with torch.no_grad():
                ...
                # img = img.half() if model.half() else img.float()  # uint8 to fp16/32 #half不支持
                img = img.float()  # uint8 to fp16/32 #half不支持
...
model = attempt_load(weights, map_location=device)
print(">> attempt_load() finished!")
model.to(device).eval()
print(">> to(device).eval() finished!")

# model.half() #TODO: 在CPU环境中half不可用！
